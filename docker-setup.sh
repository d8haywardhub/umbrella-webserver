#!/bin/sh
cd ~/Dev
docker container ls -as
# stop mongoDB 
service mongodb stop
# stop and remove containers
docker stop umbrella-backend-project
docker stop webserver
docker rm umbrella-backend-project
docker rm webserver
# remove images
docker image ls -a
docker rmi umbrella-backend_app
docker rmi umbrella-webserver-project:prod
docker rmi umbrella-webserver-project:dev
# pruning
docker system prune -f
# show status
docker image ls -a
docker container ls -as
# build webserver prod and dev images
cd ~/Dev/Angular/umbrella-webserver
docker build -t umbrella-webserver-project:prod .
docker build -t umbrella-webserver-project:dev --build-arg configuration="" .
# show running images
cd ~/Dev
docker image ls -a
docker container ls -as

